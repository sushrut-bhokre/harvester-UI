...
export default {
  ...
  computed: {
    secretOption() {
      ...
      out.unshift({
        label: this.t('zeus.virtualMachine.createSSHKey'),
        value: _NEW,
      });
  ...
  methods: {
    async saveInventory() {
      if (this.enableInventory) {
        ...
        if (!this.value.spec.baseboardSpec.connection.host) {
          errors.push(this.t('validation.required', { key: this.t('zeus.seeder.inventory.host.label') }, true));
        }

        if (!this.value.spec.baseboardSpec.connection.port) {
          errors.push(this.t('validation.required', { key: this.t('zeus.seeder.inventory.port.label') }, true));
        }

        if (!this.selectedSecret) {
          errors.push(this.t('validation.required', { key: this.t('zeus.seeder.inventory.secret.label') }, true));
        }
        ...
        if (!this.value.id) {
          this.value.metadata.annotations['metal.zeushci.io/local-node-name'] = this.node.id;
        }

        this.value.metadata.annotations['metal.zeushci.io/local-inventory'] = 'true';
    ...
    async saveSecret(buttonCb) {
      ...
      if (!this.username) {
        this.errors.push(this.t('validation.required', { key: this.t('zeus.virtualMachine.input.username') }, true));
      }

      if (!this.password) {
        this.errors.push(this.t('validation.required', { key: this.t('zeus.virtualMachine.input.password') }, true));
      }
    ...
    <div v-if="enableInventory">
      <div class="row mt-10">
        <div class="col span-6">
          <LabeledInput
            ...
            :label="t('zeus.seeder.inventory.host.label')"
            :placeholder="t('zeus.seeder.inventory.host.placeholder')"
            :mode="mode"
            required
          />
          <Checkbox
            ...
            class="mt-5"
            :mode="mode"
            :label="t('zeus.seeder.inventory.insecureTLS.label')"
          />
        </div>
        <div class="col span-6">
          <LabeledInput
            v-model:value.number="value.spec.baseboardSpec.connection.port"
            :label="t('zeus.seeder.inventory.port.label')"
            :placeholder="t('zeus.seeder.inventory.port.placeholder')"
            :mode="mode"
            required
          />
        </div>
      ...
      <div class="row mt-20">
        <div class="col span-6">
          <LabeledSelect
            ...
            :label="t('zeus.seeder.inventory.secret.label')"
            :mode="mode"
      ...
      <div class="row mt-20">
        <div class="col span-6">
          <RadioGroup
            ...
            name="enabled"
            :options="[true, false]"
            :label="t('zeus.seeder.inventory.event.label')"
      ...
      >
        <LabeledInput
          ...
          :label="t('zeus.seeder.inventory.pollingInterval.label')"
          :mode="mode"
        />
      </div>
      ...
      >
        <template #title>
          {{ t('zeus.seeder.inventory.secret.create.title') }}
        </template>
        ...
        <template #content>
          ...
          <LabeledInput
            ...
            :label="t('zeus.virtualMachine.input.username')"
            class="mb-20"
            required
          />
          ...
          <LabeledInput
            ...
            type="password"
            :label="t('zeus.virtualMachine.input.password')"
            class="mb-20"
            required
          />
        </template>
