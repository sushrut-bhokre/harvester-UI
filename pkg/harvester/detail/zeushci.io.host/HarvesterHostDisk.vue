...
export default {
  ...
  computed: {
    ...
    provisioner() {
      let labelKey = `zeus.host.disk.storage.longhorn.${ LONGHORN_VERSION_V1 }.label`;

      if (this.value?.blockDevice?.spec?.provisioner.longhorn) {
        labelKey = `zeus.host.disk.storage.longhorn.${ this.value.blockDevice.spec.provisioner.longhorn.engineVersion }.label`;
      }

      if (this.value?.blockDevice?.spec?.provisioner.lvm) {
        labelKey = 'zeus.host.disk.storage.lvm.label';
      }
...
<template>
  ...
  >
      ...
      <div class="row">
        <div class="col span-12">
          <LabelValue
            ...
            :name="t('zeus.host.disk.tags.label')"
          >
            <template #value>
              <div class="mt-5">
                <Tag
    ...
    <div class="row mt-10">
      ...
      >
        <LabelValue
          :name="t('zeus.host.disk.path.label')"
          :value="value.path"
        />
      </div>
      <div class="col span-4">
        <LabelValue
          :name="t('zeus.host.disk.provisioner')"
          :value="provisioner"
        />
      </div>
    </div>
