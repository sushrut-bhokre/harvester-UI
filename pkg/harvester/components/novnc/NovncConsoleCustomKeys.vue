...
export default {
  ...
  computed: {
    savedShortcutKeys() {
      ...
      try {
        ...
        this.$store.dispatch('growl/fromError', {
          title: this.t('generic.notification.title.error', { name: this.t('zeus.virtualMachine.detail.console.customShortcutKeys') }),
          err,
        }, { root: true });
      }
    ...
    recordButton() {
      if (this.isRecording) {
        return 'zeus.virtualMachine.detail.console.record.stop';
      }

      return 'zeus.virtualMachine.detail.console.record.start';
    },

    keysRecordFormat() {
      if (!this.isRecording && this.keysRecord.length === 0) {
        return this.t('zeus.virtualMachine.detail.console.record.tips');
      }
...
<template>
  ...
  >
    <template #title>
      <t k="zeus.virtualMachine.detail.console.customShortcutKeys" />
    </template>
    ...
    <template #content>
      ...
      <div class="displayed-keys mt-20">
        <h4
          v-clean-html="t('zeus.virtualMachine.detail.console.record.preferredKeys')"
