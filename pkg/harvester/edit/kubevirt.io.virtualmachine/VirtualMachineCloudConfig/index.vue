...
export default {
  ...
  async fetch() {
    ...
    optionUser.unshift({
      label: this.t('zeus.virtualMachine.cloudConfig.createNew'),
      value: _NEW,
    });
  ...
  computed: {
    ...
    createTypeLable() {
      const label = `zeus.virtualMachine.cloudConfig.${ this.templateType }.label`;
  ...
  methods: {
    ...
    async save(buttonCb) {
      ...
      if (!this.cloudTemplateName) {
        this.errors.push(this.t('validation.required', { key: this.t('zeus.virtualMachine.input.name') }, true));
        buttonCb(false);
      ...
      if (!this.cloudTemplate) {
        const keyLabel = this.templateType === 'user' ? 'zeus.cloudTemplate.userData' : 'zeus.cloudTemplate.networkData';
...
<template>
  <div>
    <h2 v-if="!isView">
      {{ t('zeus.virtualMachine.cloudConfig.title') }}
    </h2>
    ...
    >
      <template #title>
        {{ t('zeus.virtualMachine.cloudConfig.createTemplateTitle', { name: createTypeLable }) }}
      </template>

      <template #content>
        <LabeledInput
          v-model:value="cloudTemplateName"
          :label="t('zeus.virtualMachine.input.name')"
          class="mb-20"
          required
