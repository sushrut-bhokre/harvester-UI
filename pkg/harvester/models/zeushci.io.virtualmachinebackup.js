...
export default class HciVmBackup extends HarvesterResource {
  detailPageHeaderActionOverride(realMode) {
    ...
    if (realMode === _CREATE) {
      return route.params.resource === HCI.BACKUP ? this.t('zeus.backup.title') : this.t('zeus.vmSnapshot.title');
    }
  }
    ...
    return [
      {
        ...
        label:   this.t('zeus.action.restoreExistingVM'),
      },
      {
        action:  'restoreNewVM',
        enabled: canCreateVM && this?.status?.readyToUse,
        icon:    'icons icon-backup',
        label:   this.t('zeus.action.restoreNewVM'),
      },
      ...out
    ];
  }
