  ...
  methods: {
    ...
    addUser(buttonCb) {
      ...
      if (!reg.test(this.newUsername)) {
        const message = this.t('zeus.virtualMachine.accessCredentials.invalidUser');
      ...
      if (this.userOptions.includes(this.newUsername)) {
        const message = this.t('zeus.virtualMachine.accessCredentials.duplicatedUser');
...
<template>
  <div class="vm__credentials-users">
    <LabeledSelect
      ...
      :options="userOptions"
      :label="t('zeus.virtualMachine.accessCredentials.injectSSH.users')"
      :taggable="multiple"
      :mode="mode"
      :multiple="multiple"
      :searchable="false"
      :tooltip="t('zeus.virtualMachine.accessCredentials.userTips')"
    ...
    >
      <template #title>
        {{ t('zeus.virtualMachine.accessCredentials.injectSSH.addUser') }}
      </template>
