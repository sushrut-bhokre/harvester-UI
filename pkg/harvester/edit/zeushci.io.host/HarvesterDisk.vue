...
import { LVM_DRIVER } from '../../models/storage.k8s.io.storageclass';
...
export default {
  ...
  computed: {
    provisioners() {
      ...
      csiDrivers.forEach(({ name }) => {
        ...
        case LONGHORN_DRIVER:
          out.push({
            label: `zeus.host.disk.storage.longhorn.${ LONGHORN_VERSION_V1 }.label`,
          ...
          if (this.longhornSystemVersion === LONGHORN_VERSION_V2 || this.value.provisionerVersion === LONGHORN_VERSION_V2) {
            out.push({
              label:    `zeus.host.disk.storage.longhorn.${ LONGHORN_VERSION_V2 }.label`,
        ...
        case LVM_DRIVER:
          out.push({
            label: 'zeus.host.disk.storage.lvm.label',
            value: name,
          });
          break;
        }
    ...
    lvmVolumeGroups() {
      ...
      out.unshift({
        label: this.t('zeus.host.disk.lvmVolumeGroup.create'),
        value: _NEW,
      });
    ...
    schedulableTooltipMessage() {
      ...
      if (this.targetDisk && !this.targetDisk.allowScheduling && name && path) {
        return this.t('zeus.host.disk.allowScheduling.tooltip', { name, path });
      } else {
    ...
    formattedBannerLabel() {
      ...
      const label = this.t('zeus.host.disk.lastFormattedAt.info');

      if (system) {
        return `${ label } ${ this.t('zeus.host.disk.fileSystem.info', { system }) }`;
      } else {
        return label;
      }
    },
    ...
    provisionerTooltip() {
      if (
        ...
        return this.t('zeus.storage.storageClass.longhorn.versionTooltip');
      }
...
<template>
  ...
  >
    ...
    <Banner
      v-if="isFormatting"
      color="info"
      :label="t('zeus.host.disk.fileSystem.formatting')"
    />
    <Banner
    ...
    <div v-if="!value.isNew">
      <div class="row">
        <div class="col span-12">
          <Tags
            ...
            :label="t('zeus.host.disk.tags.label')"
            :add-label="t('zeus.host.disk.tags.addLabel')"
            :mode="mode"
          />
        </div>
      </div>
        ...
        <div class="col span-12">
          <div class="pull-left">
            <RadioGroup
              ...
              name="diskScheduling"
              :label="t('zeus.host.disk.allowScheduling.label')"
              :mode="mode"
      ...
      >
        <div class="col flex span-12">
          <LabelValue
            :name="t('zeus.host.disk.storageAvailable.label')"
            :value="value.storageAvailable"
          />
          <LabelValue
            :name="t('zeus.host.disk.storageScheduled.label')"
            :value="value.storageScheduled"
          />
          <LabelValue
            :name="t('zeus.host.disk.storageMaximum.label')"
    ...
    <div class="row mt-10">
      <div :class="`col span-${ value.isNew ? '6': '12' }`">
        <LabeledSelect
          ...
          :mode="mode"
          label-key="zeus.host.disk.provisioner"
      ...
      >
        <RadioGroup
          ...
          :mode="mode"
          name="forceFormatted"
          label-key="zeus.host.disk.forceFormatted.label"
          :labels="[t('generic.no'),t('zeus.host.disk.forceFormatted.yes')]"
          :options="[false, true]"
          :disabled="forceFormattedDisabled"
          tooltip-key="zeus.host.disk.forceFormatted.toolTip"
        >
        </RadioGroup>
      </div>
      <div
      ...
      >
        <LabeledSelect
          ...
          :mode="mode"
          label-key="zeus.host.disk.lvmVolumeGroup.label"
          :localized-label="true"
          :searchable="false"
          :taggable="true"
          :multiple="false"
    ...
    >
      <template #title>
        {{ t('zeus.host.disk.lvmVolumeGroup.label') }}
      </template>
