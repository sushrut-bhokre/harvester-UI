...
export default class USBDevice extends SteveModel {
  ...
  async disablePassthrough() {
    ...
    try {
      if (!this.claimedByMe) {
        throw new Error(this.$rootGetters['i18n/t']('zeus.usb.cantUnclaim', { name: escapeHtml(this.metadata.name) }));
      } else {
        await this.passthroughClaim.remove();
      }
    } catch (err) {
      this.$dispatch('growl/fromError', {
        title: this.$rootGetters['i18n/t']('zeus.usb.unclaimError', { name: escapeHtml(this.metadata.name) }),
        err,
      }, { root: true });
    }
  }
  ...
  showDetachWarning() {
    this.$dispatch('growl/warning', {
      title:   this.$rootGetters['i18n/t']('zeus.usb.detachWarning.title'),
      message: this.$rootGetters['i18n/t']('zeus.usb.detachWarning.message'),
      timeout: 5000
    }, { root: true });
  }
