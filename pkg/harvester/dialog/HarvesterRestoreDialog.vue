...
export default {
  ...
  methods: {
    ...
    async saveRestore(buttonCb) {
      ...
      if (!this.backupName) {
        this['errors'] = [
          this.t('zeus.modal.restore.message.backup')
        ];
        buttonCb(false);
        ...
        if (res._status === 200 || res._status === 204) {
          this.$store.dispatch(
            'growl/success',
            {
              ...
              message: this.t('zeus.modal.restore.success', { name: this.backupName })
            },
            { root: true }
          );
...
<template>
  <Card :show-highlight-border="false">
    <template #title>
      {{ t('zeus.modal.restore.title') }}
    </template>

    <template #body>
      <LabeledSelect
        v-model:value="backupName"
        :label="t('zeus.modal.restore.selectBackup')"
