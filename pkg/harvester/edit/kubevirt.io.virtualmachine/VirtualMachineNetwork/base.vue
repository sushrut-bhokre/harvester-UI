...
<template>
  <div>
    <div class="row mb-20">
      ...
      >
        <InputOrDisplay
          :name="t('zeus.fields.model')"
          :value="value.model"
          :mode="mode"
        >
          <LabeledSelect
            v-model:value="value.model"
            :label="t('zeus.fields.model')"
            :disabled="isDisabled"
            :options="modelOption"
            :mode="mode"
            required
    ...
    <div v-if="!isMasquerade && isSingle">
      <div class="row mb-20">
        ...
        >
          {{ showAdvanced ? t('zeus.generic.hideMore') : t('zeus.generic.showMore') }}
        </a>
      </div>
      ...
      <div class="row">
        ...
        >
          <InputOrDisplay
            :name="t('zeus.fields.macAddress')"
            :value="value.macAddress"
            :mode="mode"
          >
            <LabeledInput
              v-model:value="value.macAddress"
              label-key="zeus.fields.macAddress"
              :mode="mode"
              :tooltip="t('zeus.virtualMachine.volume.macTip')"
              @update:value="update"
            />
          </InputOrDisplay>
        </div>
