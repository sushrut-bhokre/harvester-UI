      ...
      try {
        ...
        if (res._status === 200 || res._status === 204) {
          this.$store.dispatch('growl/success', {
            ...
            message: this.t('zeus.modal.exportImage.message.success', { name: this.name })
          }, { root: true });
...
<template>
  <Card :show-highlight-border="false">
    <template #title>
      {{ t('zeus.modal.exportImage.title') }}
    </template>

    <template #body>
      <LabeledSelect
        v-model:value="namespace"
        :label="t('zeus.modal.exportImage.namespace')"
        :options="namespaces"
        class="mb-20"
        required
      />
      ...
      <LabeledInput
        v-model:value="name"
        :label="t('zeus.modal.exportImage.name')"
        required
      />
