...
export default class HciNode extends HarvesterResource {
  get _availableActions() {
    const cordon = {
      ...
      label: this.t('zeus.action.cordon'),
      total: 1
    };
    ...
    const uncordon = {
      ...
      label: this.t('zeus.action.uncordon'),
      total: 1
    };
    ...
    const enableMaintenance = {
      ...
      label: this.t('zeus.action.enableMaintenance'),
      total: 1
    };
    ...
    const disableMaintenance = {
      ...
      label: this.t('zeus.action.disableMaintenance'),
      total: 1
    };
    ...
    const enableCPUManager = {
      ...
      label: this.t('zeus.action.enableCPUManager'),
      total: 1
    };
    ...
    const disableCPUManager = {
      ...
      icon: 'icon icon-fw icon-os-management',
      label: this.t('zeus.action.disableCPUManager'),
      total: 1
    };
    ...
    const shutDown = {
      ...
      label: this.t('zeus.action.shutdown'),
      total: 1
    };
    ...
    const powerOn = {
      ...
      enabled: this.hasAction('powerActionPossible') && this.hasAction('powerAction') && this.isStopped && !!this.inventory,
      icon: 'icon icon-fw icon-play',
      label: this.t('zeus.action.powerOn'),
      total: 1
    };
    ...
    const reboot = {
      ...
      label: this.t('zeus.action.reboot'),
      total: 1
    };
  ...
  get stateDescription() {
    ...
    if (initIP && currentIP && currentIP !== initIP) {
      return this.t('zeus.host.inconsistentIP', { currentIP, initIP });
    }
  ...
  get blockDevices() {
    ...
    return blockDevices.filter((s) => {
      const bdNodeName = s?.spec?.nodeName;

      return bdNodeName && (nodeId.includes(bdNodeName) || bdNodeName.includes(nodeId));
    });
  }
  ...
  get diskStatusCount() {
    const allBlockDevices = this.blockDevices || [];
    const parentBlockDevices = allBlockDevices.filter(bd => bd.isParent) || [];
    const physicalDisks = parentBlockDevices.length > 0 ? parentBlockDevices : allBlockDevices;
    const lhDisks = this.longhornDisks || [];

    const total = Math.max(physicalDisks.length, lhDisks.length);
    ...
    parentBlockDevices.forEach((d) => {
      const isHealthy = !d.metadata?.state?.error;
      const isProvisioned = d.status?.provisionPhase === 'Provisioned';

      if (!isHealthy && !isProvisioned) {
        errorCount++;
      }
    });
  ...
  async shutDown(resources = this) {
    try {
      ...
      await this.$dispatch('growl/success', {
        ...
        message: this.t('zeus.host.powerAction.message.success', {
          name: this.name,
          operation: 'shut down'
        })
      }, { root: true });
