...
import { UNIT_SUFFIX } from '@pkg/harvester/utils/unit';
  ...
  computed: {
    ...
    accessibleResources() {
      ...
      return RESOURCES.filter((resource) => {
        ...
        return this.$store.getters[`${ inStore }/schemaFor`](resource.type);
      });
    },
    totalCountGaugeInput() {
      ...
      this.accessibleResources.forEach((resource) => {
        ...
        if (resource.type === HCI.BLOCK_DEVICE) {
          ...
          (this.nodes || []).map((node) => {
            total += node.diskStatusCount?.total || 0;
            errorCount += node.diskStatusCount?.errorCount || 0;
          });
  ...
  <section v-else>
    ...
    <div v-if="!enabledMonitoringAddon && canEnableMonitoringAddon">
      <Banner color="info">
        <MessageLink
          ...
          prefix-label="zeus.monitoring.alertmanagerConfig.disabledAddon.prefix"
          middle-label="zeus.monitoring.alertmanagerConfig.disabledAddon.middle"
          suffix-label="zeus.monitoring.alertmanagerConfig.disabledAddon.suffix"
        />
      </Banner>
    </div>
    ...
    <template v-if="nodes.length && hasMetricNodeSchema">
      ...
      >
        ...
        <HardwareResourceGauge
          :name="t('zeus.dashboard.hardwareResourceGauge.cpu')"
          :reserved="cpuReserved"
          :used="cpuUsed"
        />
        <HardwareResourceGauge
          :name="t('zeus.dashboard.hardwareResourceGauge.memory')"
          :reserved="ramReserved"
          :used="ramUsed"
        />
        <HardwareResourceGauge
          v-if="hasLonghornSchema"
          :name="t('zeus.dashboard.hardwareResourceGauge.storage')"
          :used="storageUsed"
          :reserved="storageAllocated"
          :reserved-title="t('zeus.dashboard.hardwareResourceGauge.allocated')"
        />
      </div>
    </template>
    ...
    >
      ...
      <Tab
        v-if="showVmMetrics"
        name="vm-metric"
        :label="t('zeus.dashboard.sections.vmMetrics.label')"
        :weight="98"
      >
        <template #default="props">
          <DashboardMetrics
