  ...
  computed: {
    ...
    ntpSyncedStatus() {
      ...
      if (status === 'disabled') {
        return {
          status:  'disabled',
          warning: { key: 'zeus.host.ntp.ntpSyncStatus.isDisabled' }
        };
      }
      ...
      if (status === 'unsynced') {
        return {
          status:  'unsynced',
          warning: {
            key:     'zeus.host.ntp.ntpSyncStatus.isUnsynced',
...
<template>
  ...
  <div v-else>
    ...
    >
      ...
      <Tab
        name="instance"
        :label="t('zeus.host.tabs.instance')"
        :weight="3"
        class="bordered-table"
      >
        <Instance :node="value" />
      ...
      <Tab
        ...
        name="network"
        :label="t('zeus.host.tabs.network')"
        :weight="2"
        class="bordered-table"
      >
        <InfoBox
      ...
      <Tab
        label-key="zeus.virtualMachine.detail.tabs.events"
        name="events"
        :weight="-99"
      >
        <SortableTable
