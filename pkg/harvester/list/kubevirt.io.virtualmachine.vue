  ...
  computed: {
    headers() {
      const restoreCol = {
        ...
        labelKey:  'zeus.tableHeaders.restore',
      ...
      const nodeCol = {
        ...
        labelKey:  'zeus.tableHeaders.vm.node'
      };
      ...
      handler(neu) {
        ...
        if (count > 0 && vmNames.length > 0) {
          this.$store.dispatch('growl/warning', {
            title:   this.t('zeus.notification.restartRequired.title', { count }),
            message: this.t('zeus.notification.restartRequired.message', { vmNames: vmNames.join(', ') }),
            timeout: 10000,
          }, { root: true });
  ...
  methods: {
    lockIconTooltipMessage(row) {
      ...
      if (row.encryptedVolumeType === 'all') {
        return this.t('zeus.virtualMachine.volume.lockTooltip.all');
      } else if (row.encryptedVolumeType === 'partial') {
        return this.t('zeus.virtualMachine.volume.lockTooltip.partial');
      }
