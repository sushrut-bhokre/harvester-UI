...
export default {
  ...
  computed: {
    ...
    historyUsersOptions() {
      ...
      out.unshift({
        label: this.t('zeus.virtualMachine.accessCredentials.injectSSH.addUser'),
        value: _NEW,
      });
  ...
  methods: {
    ...
    headerFor(type) {
      return {
        userPassword: this.$store.getters['i18n/t']('zeus.virtualMachine.accessCredentials.resetPwd.label'),
        sshPublicKey: this.$store.getters['i18n/t']('zeus.virtualMachine.accessCredentials.injectSSH.label'),
      }[type];
    },
  }
};
...
<template>
  <div>
    <Banner
      class="mb-20"
      color="info"
      :label="t('zeus.virtualMachine.accessCredentials.tips')"
    />
    ...
    >
      ...
      >
        {{ t('zeus.virtualMachine.accessCredentials.resetPwd.label') }}
      </button>
      ...
      >
        {{ t('zeus.virtualMachine.accessCredentials.injectSSH.label') }}
      </button>
    </div>
  </div>
</template>
