...
export default {
  ...
  methods: {
    ...
    groupLabel(group) {
      ...
      if (row.isFake) {
        return `${ this.t('zeus.network.clusterNetwork.label') }: ${ row.nameDisplay }`;
      }

      return `${ this.t('zeus.network.clusterNetwork.label') }: ${ group.key }`;
    },
  },
};
</script>
  ...
  <div>
    ...
    <div v-else>
      <Masthead
        ...
        :type-display="t('zeus.clusterNetwork.title')"
        :resource="HCI.CLUSTER_NETWORK"
        ...
        <template #group-by="{group}">
          <div class="group-bar">
            ...
            <div class="right">
              ...
              >
                {{ t('zeus.vlanConfig.createNetworkConfig') }}
              </router-link>
              <button
                type="button"
        ...
        >
          ...
          >
            <td
              ...
              colspan="12"
            >
              {{ clusterNetwork.id === 'mgmt' ? t('zeus.clusterNetwork.mgmt') : t('zeus.clusterNetwork.clusterNetwork') }}
            </td>
          </tr>
        </template>
      </ResourceTable>
