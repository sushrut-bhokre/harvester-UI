  ...
  methods: {
    ...
    willSave() {
      ...
      if (ciphers && protocols.length === 0) {
        errors.push(this.t('validation.required', { key: this.t('zeus.sslParameters.protocols.label') }, true));
      }

      if (!ciphers && protocols.length > 0) {
        errors.push(this.t('validation.required', { key: this.t('zeus.sslParameters.ciphers.label') }, true));
      }

      const regex = /^(:?[A-Z0-9]+(?:-[A-Z0-9]+)+)+$/gm;

      if (ciphers && (!ciphers.match(regex) || ciphers.startsWith(':'))) {
        errors.push(this.t('validation.invalid', { key: this.t('zeus.sslParameters.ciphers.label') }, true));
      }
    ...
    <div class="row mt-10">
      <div class="col span-12">
        <LabeledSelect
          ...
          :mode="mode"
          label-key="zeus.sslParameters.protocols.label"
          :multiple="true"
    ...
    <div class="row mt-20">
      <div class="col span-12">
        <LabeledInput
          ...
          :mode="mode"
          label-key="zeus.sslParameters.ciphers.label"
          @update:value="update"
        />
      </div>
    </div>
