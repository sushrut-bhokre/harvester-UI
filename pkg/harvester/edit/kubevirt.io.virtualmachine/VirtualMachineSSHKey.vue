...
export default {
  ...
  methods: {
    ...
    async save(buttonCb) {
      ...
      if (!this.sshName) {
        const fieldName = this.t('zeus.virtualMachine.input.name');
      ...
      if (this.sshName.length > 63) {
        const message = this.t('zeus.validation.custom.tooLongName', { max: 63 });
...
<template>
  <div>
    <LabeledSelect
      ...
      :label="t('zeus.virtualMachine.input.sshKey')"
      :taggable="!disabled"
      :mode="mode"
      :multiple="true"
    ...
    >
      <template #title>
        {{ t('zeus.virtualMachine.sshTitle') }}
      </template>
      ...
      <template #content>
        ...
        <LabeledInput
          ...
          :label="t('zeus.virtualMachine.input.sshKeyValue')"
          :min-height="160"
          class="mb-20"
          type="multiline"
          required
