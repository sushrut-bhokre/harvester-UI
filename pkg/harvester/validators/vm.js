...
import { isValidMac, isValidDNSLabelName } from '@pkg/harvester/utils/regular';
import { SOURCE_TYPE } from '@pkg/harvester/config/harvester-map';
...
export function vmNetworks(spec, getters, errors, validatorArgs) {
  ...
  interfaces.map((I, index) => {
    ...
    const type = getters['i18n/t']('zeus.fields.network');

    const lowerType = getters['i18n/t']('zeus.validation.vm.network.lowerType');
    const upperType = getters['i18n/t']('zeus.validation.vm.network.upperType');

    validName(getters, errors, I.name, networkNames, prefix, type, lowerType, upperType);

    if (N.multus) {
      if (!N.multus.networkName) {
        const key = getters['i18n/t']('zeus.fields.network');
        const message = getters['i18n/t']('validation.required', { key });

        errors.push(getters['i18n/t']('zeus.validation.generic.tabError', { prefix, message }));
      }
    }

    if (I.macAddress && !isValidMac(I.macAddress) && !N.pod) {
      const message = getters['i18n/t']('zeus.validation.vm.network.macFormat');
...
export function vmDisks(spec, getters, errors, validatorArgs, displayKey, value) {
  ...
  _disks.forEach((D, idx) => {
    ...
    if (!D.disk && !D.cdrom) {
      const key = getters['i18n/t']('zeus.fields.type');
      const message = getters['i18n/t']('validation.required', { key });

      errors.push(getters['i18n/t']('zeus.validation.generic.tabError', { prefix, message }));
    }

    const type = getters['i18n/t']('zeus.fields.volume');
    const lowerType = getters['i18n/t']('zeus.validation.vm.volume.lowerType');
    const upperType = getters['i18n/t']('zeus.validation.vm.volume.upperType');
  ...
  _volumes.forEach((V, idx) => {
    ...
    if (type === SOURCE_TYPE.NEW || type === SOURCE_TYPE.IMAGE) {
      if (!/([1-9]|[1-9][0-9]+)[a-zA-Z]+/.test(typeValue?.spec?.resources?.requests?.storage)) {
        const key = getters['i18n/t']('zeus.fields.size');
        const message = getters['i18n/t']('validation.required', { key });
      ...
      if (typeValue?.spec?.resources?.requests?.storage && !/^([0-9][0-9]{0,8})[a-zA-Z]+$/.test(typeValue?.spec?.resources?.requests?.storage)) {
        const message = getters['i18n/t']('zeus.validation.generic.maximumSize', { max: `999999999 ${GIBIBYTE}` });

        errors.push(getters['i18n/t']('zeus.validation.generic.tabError', { prefix, message }));
      }

      if (type === SOURCE_TYPE.IMAGE && !typeValue?.spec?.storageClassName && !isVMTemplate) { // type === SOURCE_TYPE.IMAGE
        const key = getters['i18n/t']('zeus.fields.image');
        const message = getters['i18n/t']('validation.required', { key });
      ...
      if (!typeValue?.spec?.storageClassName && V?.persistentVolumeClaim?.claimName && type !== SOURCE_TYPE.IMAGE) {
        const key = getters['i18n/t']('zeus.fields.storageClass');
        const message = getters['i18n/t']('validation.required', { key });
    ...
    if (type === SOURCE_TYPE.ATTACH_VOLUME) {
      ...
      if (!hasExistingVolume && selectedVolumeName) { // selected volume may have been deleted. e.g: use template
        ...
        errors.push(getters['i18n/t']('zeus.validation.generic.hasDelete', { type, name: selectedVolumeName }));
      }

      if (!selectedVolumeName) { // volume is not selected.
        const key = getters['i18n/t']('zeus.virtualMachine.volume.volume');
    ...
    if (type === SOURCE_TYPE.CONTAINER && !V.containerDisk.image) {
      const key = getters['i18n/t']('zeus.fields.dockerImage');
      const message = getters['i18n/t']('validation.required', { key });
  ...
  if ((!requiredVolume || _volumes.length === 0) && !value.links) {
    errors.push(getters['i18n/t']('zeus.validation.vm.volume.needImageOrExisting'));
  }
...
function getVolumeType(getters, V, DVTS, value) {
  ...
  if (V.persistentVolumeClaim) {
    ...
    outValue = DVTS.find((DVT) => {
      return V.persistentVolumeClaim.claimName === DVT.metadata.name && DVT.metadata?.annotations && Object.prototype.hasOwnProperty.call(DVT.metadata.annotations, 'zeushci.io/imageId');
    });
...
function validName(getters, errors, name, names = [], prefix, type, lowerType, upperType) {
  ...
  if (names.findIndex((N) => name === N) !== -1) {
    errors.push(getters['i18n/t']('zeus.validation.vm.duplicatedName', { type, name }));
  }
  ...
  if (name && name?.length > maxNameLength) {
    const key = getters['i18n/t']('zeus.fields.name');
    const message = getters['i18n/t']('zeus.validation.generic.maxLength', { key, max: maxNameLength });
  ...
  if (!isValidDNSLabelName(name)) {
    ...
    errors.push(getters['i18n/t']('zeus.validation.generic.regex', {
