...
export default class HciLB extends HarvesterResource {
  ...
  get workloadTypeDisplay() {
    ...
    return this.$rootGetters['i18n/t'](`zeus.loadBalancer.workloadType.options.${ workloadType }`);
  }

  get ipamDisplay() {
    const ipam = this.spec?.ipam;

    return this.$rootGetters['i18n/t'](`zeus.loadBalancer.ipam.options.${ ipam }`);
  }
  ...
  get details() {
    const out = [{
      label:   this.t('zeus.loadBalancer.ipam.label'),
      content: this.ipamDisplay,
    }, {
      label:   this.t('zeus.loadBalancer.workloadType.label'),
