...
const NAMESPACE_SELECTOR = 'loadbalancer.zeushci.io/namespace';
const PROJECT_SELECTOR = 'loadbalancer.zeushci.io/project';
...
export default {
  ...
  computed: {
    ipamOption() {
      return [{
        label: this.t('zeus.loadBalancer.ipam.options.dhcp'),
        value: 'dhcp',
      }, {
        label: this.t('zeus.loadBalancer.ipam.options.pool'),
        value: 'pool',
      }];
    },
    ...
    ipPoolOptions() {
      ...
      return [{
        label: this.t('zeus.loadBalancer.ipPool.options.none'),
        value: '',
      }, ...out];
    },
    ...
    healthCheckPortInUseWarning() {
      ...
      if (healthCheckPort && portInUse) {
        return this.t('zeus.loadBalancer.healthCheck.warning.portInUse', { port: portInUse.backendPort }, true);
      }
...
<template>
  ...
  >
    ...
    >
      <Tab
        name="basic"
        :label="t('zeus.loadBalancer.tabs.basic')"
        :weight="99"
        ...
        <div class="row">
          <div class="col span-6">
            <LabeledSelect
              ...
              :label="t('zeus.loadBalancer.ipam.label')"
              :options="ipamOption"
              :mode="mode"
            />
          </div>
          ...
          >
            <LabeledSelect
              ...
              :label="t('zeus.loadBalancer.ipPool.label')"
      ...
      <Tab
        ...
        name="listeners"
        :label="t('zeus.loadBalancer.tabs.listeners')"
        :weight="98"
        class="bordered-table"
      >
        <Banner
      ...
      <Tab
        v-if="value.spec.workloadType === 'vm'"
        name="backendServer"
        :label="t('zeus.loadBalancer.tabs.backendServer')"
        :weight="97"
        class="bordered-table"
      >
        <div class="row">
          <div class="col span-12">
            <Banner :color="(matchingVMs.none ? 'warning' : 'success')">
              <span v-clean-html="t('zeus.loadBalancer.backendServerSelector.matchingVMs.matchesSome', matchingVMs)" />
            </Banner>
          </div>
        </div>
        <KeyValue
      ...
      <Tab
        v-if="value.spec.workloadType === 'vm'"
        name="healthCheck"
        :label="t('zeus.loadBalancer.tabs.healthCheck')"
        :weight="96"
        class="bordered-table"
      >
        <HealthCheck
