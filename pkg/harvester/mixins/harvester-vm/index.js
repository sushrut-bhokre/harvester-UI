...
export default {
  ...
  computed: {
      ...
      return nodes.filter((N) => !N.isUnSchedulable && N.isEtcd !== 'true').map((node) => {
        ...
        if (clusterNetworks.length > 0) {
          clusterNetworks.map((clusterNetwork) => {
            requireLabelKeys.push(`network.zeushci.io/${ clusterNetwork }`);
          });
        }
        ...
        return {
          label:    isNetworkSchedule ? node.nameDisplay : `${ node.nameDisplay } (${ this.t('zeus.virtualMachine.scheduling.networkNotSupport') })`,
          value:    node.id,
    ...
    affinityLabels() {
      return {
        namespaceInputLabel:      this.t('zeus.virtualMachine.affinity.namespaces.label'),
        namespaceSelectionLabels: [
          this.t('zeus.virtualMachine.affinity.thisPodNamespace'),
          this.t('workload.scheduling.affinity.allNamespaces'),
          this.t('zeus.virtualMachine.affinity.matchExpressions.inNamespaces')
        ],
        addLabel:               this.t('zeus.virtualMachine.affinity.addLabel'),
        topologyKeyPlaceholder: this.t('zeus.virtualMachine.affinity.topologyKey.placeholder')
      };
    },
  },
  ...
  methods: {
    ...
    getMaintenanceStrategyOptionLabel(opt) {
      return this.t(`zeus.virtualMachine.maintenanceStrategy.options.${ opt.label || opt }`);
    },
    ...
    getOwnerReferencesFromVM(resource) {
      ...
      const apiVersion = this.resourceType === HCI.VM ? 'kubevirt.io/v1' : 'zeushci.io/v1beta1';
    ...
    getAccessCredentialsValidation() {
      ...
      for (let i = 0; i < this.accessCredentials.length; i++) {
        ...
        if (source === ACCESS_CREDENTIALS.RESET_PWD) {
          if (!row.username) {
            const fieldName = this.t('zeus.virtualMachine.input.username');
          ...
          if (!row.newPassword) {
            const fieldName = this.t('zeus.virtualMachine.input.password');
            const message = this.t('validation.required', { key: fieldName });
          ...
          if (!row.sshkeys || row.sshkeys.length === 0) {
            const fieldName = this.t('zeus.virtualMachine.input.sshKeyValue');
            const message = this.t('validation.required', { key: fieldName });
