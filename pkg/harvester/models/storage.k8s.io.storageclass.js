...
import { DATA_ENGINE_V1, DATA_ENGINE_V2 } from './persistentvolumeclaim';
import { isInternalStorageClass } from '../utils/storage-class';

export const LVM_DRIVER = 'lvm.driver.zeushci.io';
...
export default class HciStorageClass extends StorageClass {
  ...
  get provisionerDisplay() {
    ...
    if (this.provisioner === LONGHORN_DRIVER) {
      key = `zeus.storage.storageClass.longhorn.${this.longhornVersion}.label`;
    }

    if (this.provisioner === LVM_DRIVER) {
      key = `zeus.storage.storageClass.lvm.label`;
    }
