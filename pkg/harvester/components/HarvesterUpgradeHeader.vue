      ...
      <template #popper>
        <div class="upgrade-info mb-10">
          ...
          >
            ...
            >
              <div class="col span-12">
                <p class="state">
                  {{ t('zeus.upgradePage.repoInfo.upgradeStatus') }}: <BadgeStateFormatter
                    class="ml-5"
                    :row="latestResource"
                  />
                </p>
            ...
            >
              <div class="col span-12">
                <p class="state">
                  {{ t('zeus.upgradePage.repoInfo.logStatus') }}: <span class="error ml-5">{{ downloadLogFailReason }}</span>
                </p>
              </div>
            </div>
            ...
            <div class="row mb-5">
              <div class="col span-6">
                {{ t('zeus.upgradePage.repoInfo.os') }}: <span class="text-muted">{{ repoInfo.release.os }}</span>
              </div>

              <div class="col span-6">
                {{ t('zeus.productLabel') }}: <span class="text-muted">{{ repoInfo.release.harvester }}</span>
              </div>
            </div>

            <div class="row mb-5">
              <div class="col span-6">
                {{ t('zeus.upgradePage.repoInfo.harvesterChart') }}: <span class="text-muted">{{ repoInfo.release.harvesterChart }}</span>
              </div>

              <div class="col span-6">
                {{ t('zeus.upgradePage.repoInfo.monitoringChart') }}: <span class="text-muted">{{ repoInfo.release.monitoringChart }}</span>
              </div>
            </div>

            <div class="row mb-5">
              <div class="col span-6">
                {{ t('zeus.upgradePage.repoInfo.kubernetes') }}: <span class="text-muted">{{ repoInfo.release.kubernetes }}</span>
              </div>
          ...
          <div v-if="showImage">
            <h4>{{ t('zeus.upgradePage.upgradeImage') }}<span class="float-r text-info">{{ imageProgress }}%</span></h4>
            <PercentageBar
          ...
          <h4>{{ t('zeus.upgradePage.createRepository') }} <span class="float-r text-info">{{ repoReady.isReady ? t('zeus.upgradePage.succeeded') : t('zeus.upgradePage.pending') }}</span></h4>
          <p
          ...
          <ProgressBarList
            :title="t('zeus.upgradePage.upgradeNode')"
          ...
          <ProgressBarList
            :title="t('zeus.upgradePage.upgradeSysService')"
        ...
        <div class="footer">
          ...
          >
            ...
            ></i> {{ t('zeus.upgradePage.repoInfo.downloadLog') }}
          </button>
          ...
          <button
            ...
            @click="ignoreMessage()"
          >
            {{ t('zeus.upgradePage.dismissMessage') }}
          </button>
        </div>
      </template>
    </v-dropdown>
...
/* Harvester branding: make the logo image taller to match the product name text */
.menu-spacer.isSingleProduct .side-menu-logo {
  height: 55px;
  padding: 6px 0;
