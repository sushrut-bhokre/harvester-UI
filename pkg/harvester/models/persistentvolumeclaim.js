...
export default class HciPv extends HarvesterResource {
  ...
  get availableActions() {
    ...
    const exportImageAction = {
      ...
      icon: 'icon icon-copy',
      label: this.t('zeus.action.exportImage')
    };
    const takeSnapshotAction = {
      action: 'snapshot',
      enabled: this.hasAction('snapshot'),
      icon: 'icon icon-backup',
      label: this.t('zeus.action.snapshot'),
    };
    ...
    return [
      {
        ...
        icon: 'icon icon-backup',
        label: this.t('zeus.action.cancelExpand')
      },
      ...out
    ];
  }
  ...
  get warnDeletionMessage() {
    return this.t('zeus.volume.promptRemove.tips');
  }
  ...
  get resourceExternalLink() {
    ...
    return {
      tipsKey: 'zeus.volume.externalLink.tips',
