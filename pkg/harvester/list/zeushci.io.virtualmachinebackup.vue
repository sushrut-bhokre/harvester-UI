...
import { isBackupTargetSettingEmpty } from '@pkg/harvester/utils/setting';
import { STATE, AGE, NAME, NAMESPACE } from '@shell/config/table-headers';
import FilterVMSchedule from '../components/FilterVMSchedule';
import { HCI } from '../types';
import { allSettled } from '@pkg/harvester/utils/promise';
  ...
  computed: {
    headers() {
      const cols = [
        ...
        {
          name:      'targetVM',
          labelKey:  'zeus.tableHeaders.targetVm',
          value:     'attachVM',
          align:     'left',
      ...
      if (this.schedulingVMBackupFeatureEnabled) {
        cols.push({
          ...
          labelKey:  'zeus.tableHeaders.vmSchedule',
...
<template>
  ...
  <div v-else>
    <Masthead
      :schema="schema"
      :resource="resource"
      :create-button-label="t('zeus.backup.createText')"
    />
    ...
    >
      <MessageLink
        ...
        suffix-label="zeus.backup.message.noSetting.suffix"
      />
    ...
    <div v-else-if="canUpdate">
      ...
      >
        <MessageLink
          :to="to"
          prefix-label="zeus.backup.message.viewSetting.prefix"
          middle-label="zeus.backup.message.viewSetting.middle"
          suffix-label="zeus.backup.message.viewSetting.suffix"
        />
      </Banner>
    </div>
