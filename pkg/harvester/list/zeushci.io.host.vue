...
export default {
    ...
    headers() {
      ...
      if (this.hasMetricSchema) {
        const metricCol = [
          {
            name:          'cpu',
            labelKey:      'zeus.dashboard.hardwareResourceGauge.cpu',
            value:         'id',
          ...
          {
            name:          'memory',
            labelKey:      'zeus.dashboard.hardwareResourceGauge.memory',
            value:         'id',
      ...
      if (this.rows.every((node) => node.cpuPinningFeatureEnabled)) {
        out.push({
          name:          'cpuManager',
          labelKey:      'zeus.tableHeaders.cpuManager',
          value:         'id',
  ...
  methods: {
    ...
    consoleTooltip(row) {
      if (!row.consoleUrl) {
        return this.t('zeus.host.noConsoleUrl');
      }
