...
export default {
  ...
  methods: {
    validate() {
      ...
      const nicRequired = this.t('validation.arrayCountRequired', { key: this.t('zeus.vlanConfig.uplink.nics.label'), count: 1 }, true);
      ...
      if (nics.length === 0) {
        ...
        nics.map((n) => {
          ...
          if (option && option?.disabled) {
            errors.push(this.t('zeus.vlanConfig.uplink.nics.validate.available', { nic: n }, true));
          }
        });
      }
      ...
      if (!this.value?.spec?.clusterNetwork) {
        errors.push(this.t('validation.required', { key: this.t('zeus.network.clusterNetwork.label') }, true));
      }
    ...
    nicOptions(options) {
      ...
      nics.map((n) => {
        ...
        if ((option && option?.disabled) || !option) {
          nicErrors.push(this.t('zeus.vlanConfig.uplink.nics.validate.available', { nic: n }, true));
        }
      });
...
<template>
  ...
  >
    ...
    >
      <Tab
        name="nodeSelector"
        :label="t('zeus.vlanConfig.titles.nodeSelector')"
        :weight="99"
      >
        <div class="row">
          <div class="col span-12">
            <Banner :color="(matchingNodes.none ? 'warning' : 'success')">
              <span v-clean-html="t('zeus.vlanConfig.nodeSelector.matchingNodes.matchesSome', matchingNodes)" />
            </Banner>
          </div>
        </div>
        <div class="row">
      ...
      <Tab
        name="upLink"
        :label="t('zeus.vlanConfig.titles.uplink')"
        :weight="99"
        :show-header="false"
      >
        <div class="row mt-10">
          <div class="col span-6">
            <LabelValue
              :name="t('zeus.network.clusterNetwork.label')"
        ...
        <div class="row mt-20">
          <div class="col span-12">
            ...
            <ArrayListSelect
              ...
              :array-list-props="{
                addLabel: t('zeus.vlanConfig.uplink.nics.addLabel'),
                initialEmptyRow: true,
                title: t('zeus.vlanConfig.uplink.nics.label'),
                required: true,
                protip: false,
              }"
              :select-props="{
                placeholder: t('zeus.vlanConfig.uplink.nics.placeholder'),
              }"
            />
          </div>
        </div>
        ...
        <h3 class="mt-20">
          {{ t('zeus.vlanConfig.titles.bondOptions') }}
        </h3>
        <div class="row">
          <div class="col span-6">
            <LabeledSelect
              v-model:value="bondOptionMode"
              :label="t('zeus.vlanConfig.uplink.bondOptions.mode.label')"
              :mode="mode"
              :options="bondOptions"
              required
            />
          ...
          <div class="col span-6">
            <LabeledInput
              ...
              :label="t('zeus.vlanConfig.uplink.bondOptions.miimon.label')"
              :mode="mode"
              type="number"
              :tooltip="t('zeus.vlanConfig.uplink.bondOptions.miimon.tooltip', null, true)"
            />
          </div>
        </div>

        <h3 class="mt-20">
          {{ t('zeus.vlanConfig.titles.attributes') }}
        </h3>
        <div class="row mt-10">
          <div class="col span-6">
            <LabeledInput
              v-model:value.number="mtu"
              :label="t('zeus.vlanConfig.uplink.linkAttributes.mtu.label')"
              :mode="mode"
              type="number"
            />
          </div>
