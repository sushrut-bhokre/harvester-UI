...
export default class VirtVm extends HarvesterResource {
  get availableActions() {
    ...
    return [
      ...
      },
      {
        ...
        icon: 'icon icon-close',
        label: this.t('zeus.action.forceStop'),
        bulkable: true
      },
      {
        action: 'pauseVM',
        ...
        label: this.t('zeus.action.pause')
      },
      {
        action: 'unpauseVM',
        enabled: !!this.actions?.unpause,
        icon: 'icon icon-spinner',
        label: this.t('zeus.action.unpause')
      },
      {
        action: 'restartVM',
        enabled: !!this.actions?.restart,
        icon: 'icon icon-refresh',
        label: this.t('zeus.action.restart'),
        bulkable: true,
      ...
      },
      {
        ...
        label: this.t('zeus.action.softreboot')
      },
      {
        action: 'startVM',
        enabled: !!this.actions?.start,
        icon: 'icon icon-play',
        label: this.t('zeus.action.start'),
        bulkable: true
      },
      {
        action: 'backupVM',
  ...
  get migrationMessage() {
    if (
      ...
      return {
        ...this.actualState,
        message: this.t('zeus.modal.migration.failedMessage')
      };
    }
  ...
  get rootImageId() {
    ...
    this.volumeClaimTemplates.find((volume) => {
      imageId = volume?.metadata?.annotations?.['zeushci.io/imageId'];
    ...
    const rules = [
      {
        ...
        minLength: 1,
        maxLength: 63,
        translationKey: 'zeus.fields.name'
      },
      {
        nullable: false,
  ...
  get stateDescription() {
    if (this.isRestartRequired) {
      return this.t('zeus.virtualMachine.hotplug.restartVMMessage');
    }
